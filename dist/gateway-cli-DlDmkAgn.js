...
import { _t as DEFAULT_MODEL, c as resolveAllowedModelRef, d as resolveHooksGmailModel, gt as DEFAULT_CONTEXT_TOKENS, i as isCliProvider, l as resolveConfiguredModelRef, p as resolveThinkingDefault, r as getModelRefStatus, vt as DEFAULT_PROVIDER } from "./model-selection-qIT4GiGk.js";
...
import { c as saveMediaBuffer, l as detectMime } from "./routes-B3SUB1so.js";
...
import { a as resolveSubagentToolPolicy, i as resolveGroupToolPolicy, r as resolveEffectiveToolPolicy, t as filterToolsByPolicy } from "./pi-tools.policy-D3pfOpAb.js";
...
import { r as startBrowserControlServiceFromConfig, t as createBrowserControlContext } from "./control-service-BeD3TOdi.js";
...
import { n as createOutboundSendDeps, t as createDefaultDeps } from "./deps-DN0dKKUA.js";
...
import { c as loadAgentIdentity, d as loadAgentIdentityFromWorkspace, p as forceFreePortAndWait, r as getStatusSummary, t as runOnboardingWizard } from "./onboarding-DTdkPNbp.js";
...
import { d as resolveControlUiRootOverrideSync, f as resolveControlUiRootSync, i as discoverGatewayBeacons, n as installSkill, u as ensureControlUiAssetsBuilt } from "./onboard-skills-z9T-nHvJ.js";
import "./github-copilot-auth-BKV6Aiag.js";
import "./onboard-channels-D4jAA1kf.js";
import { r as buildChannelUiCatalog, t as applyPluginAutoEnable } from "./plugin-auto-enable-G3UvWJ3A.js";
import "./archive-BrH5WBhI.js";
import "./installs-W4Pc1LJz.js";
import { c as startHeartbeatRunner, n as formatHealthChannelLines, o as runHeartbeatOnce, r as getHealthSnapshot, s as setHeartbeatsEnabled } from "./health-format-D_VP8QJD.js";
...
import { i as probeGateway } from "./audit-BbyfC_5k.js";
...
import "./status.update-4nnd-xrd.js";
import { a as createOutboundSendDeps$1, i as resolveAgentOutboundTarget, r as resolveAgentDeliveryPlan, t as agentCommand } from "./agent-CKvXnKQe.js";
...
import "./tui-v240xX8W.js";
import { t as buildChannelAccountSnapshot } from "./status-DkW6wMxZ.js";
import "./shared-ChNOqAzp.js";
import { a as runDaemonStop, i as runDaemonStart, n as runDaemonStatus, o as runDaemonUninstall, r as runDaemonRestart, s as runDaemonInstall } from "./daemon-cli-BiIN6hQd.js";
...
async function runCronIsolatedAgentTurn(params) {
	...
	if (modelOverrideRaw !== void 0) {
		if (typeof modelOverrideRaw !== "string") return {
			status: "error",
			error: "invalid model: expected string"
		};
	...
	const skipMessagingToolDelivery = deliveryRequested && deliveryMode === "auto" && runResult.didSendViaMessagingTool === true && (runResult.messagingToolSentTargets ?? []).some((target) => matchesMessagingToolDeliveryTarget(target, {
...
function computeNextRunAtMs(schedule, nowMs) {
	if (schedule.kind === "at") return schedule.atMs > nowMs ? schedule.atMs : void 0;
...
async function ensureLoaded(state) {
	...
	for (const raw of jobs) {
		...
		if (payload && typeof payload === "object" && !Array.isArray(payload)) {
			if (migrateLegacyCronPayload(payload)) mutated = true;
		}
	}
	state.store = {
		version: 1,
...
async function runDueJobs(state) {
	...
	for (const job of due) await executeJob(state, job, now, { forced: false });
}
	...
	const finish = async (status, err, summary, outputText) => {
		...
		if (job.sessionTarget === "isolated") {
			...
			if (mode === "full") {
				...
				const maxChars = Number.isFinite(maxCharsRaw) ? Math.max(0, maxCharsRaw) : 8e3;
				const fullText = (outputText ?? "").trim();
				if (fullText) body = fullText.length > maxChars ? `${fullText.slice(0, maxChars)}…` : fullText;
			}
			const statusPrefix = status === "ok" ? prefix : `${prefix} (${status})`;
			state.deps.enqueueSystemEvent(`${statusPrefix}: ${body}`, { agentId: job.agentId });
			if (job.wakeMode === "now") state.deps.requestHeartbeatNow({ reason: `cron:${job.id}:post` });
		}
	};
	try {
		if (job.sessionTarget === "main") {
			...
			if (!text) {
				...
				await finish("skipped", kind === "systemEvent" ? "main job requires non-empty systemEvent text" : "main job requires payload.kind=\"systemEvent\"");
				return;
			}
			...
			if (job.wakeMode === "now" && state.deps.runHeartbeatOnce) {
				...
				if (heartbeatResult.status === "ran") await finish("ok", void 0, text);
				else if (heartbeatResult.status === "skipped") await finish("skipped", heartbeatResult.reason, text);
		...
		if (res.status === "ok") await finish("ok", void 0, res.summary, res.outputText);
		else if (res.status === "skipped") await finish("skipped", void 0, res.summary, res.outputText);
...
const cronHandlers = {
	...
	"cron.add": async ({ params, respond, context }) => {
		...
		respond(true, await context.cron.add(normalized), void 0);
	},
	"cron.update": async ({ params, respond, context }) => {
		...
		respond(true, await context.cron.update(jobId, p.patch), void 0);
	},
	...
	"cron.run": async ({ params, respond, context }) => {
		...
		respond(true, await context.cron.run(jobId, p.mode), void 0);
	},
...
const nodeHandlers = {
	...
	"node.event": async ({ params, respond, context, client }) => {
		...
		await respondUnavailableOnThrow(respond, async () => {
			const { handleNodeEvent } = await import("./server-node-events-BYW1rjIo.js");
...
async function startBrowserControlServerIfEnabled() {
	...
	const mod = override ? await import(override) : await import("./control-service-BJusnF18.js");
