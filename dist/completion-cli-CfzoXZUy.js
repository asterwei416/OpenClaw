...
import { n as registerSubCliByName, t as getSubCliEntries } from "./register.subclis-BPK0X-SZ.js";
...
function formatCompletionSourceLine(shell, binName, cachePath) {
	if (cachePath) return `source "${cachePath}"`;
	if (shell === "fish") return `${binName} completion --shell fish | source`;
...
async function installCompletion(shell, yes, binName = "openclaw") {
	...
	try {
		...
		const update = updateCompletionProfile(await fs.readFile(profilePath, "utf-8"), binName, cachedPath, sourceLine);
		if (!update.changed) {
		...
		if (!yes && cachedPath) console.log(`Completion cache: ${cachedPath}`);
	} catch (err) {
